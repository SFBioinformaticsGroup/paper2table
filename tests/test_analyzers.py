...
def test_jaccard_numeric_to_semantic():
    ...
    result = JaccardAnalyzer().build_mapping(cols(left), cols(right), left.rows, right.rows)
...
def test_jaccard_threshold_respected():
    ...
    assert JaccardAnalyzer(threshold=0.5).build_mapping(cols(left), cols(right), left.rows, right.rows) == {"0": "family"}
    assert JaccardAnalyzer(threshold=0.6).build_mapping(cols(left), cols(right), left.rows, right.rows) == {}


def test_alias_applies_known_alias():
    left = wrap([Row(**{"familia": "Apiaceae"})])
    right = wrap([Row(**{"family": "Apiaceae"})])
    result = AliasAnalyzer({"familia": "family"}).build_mapping(cols(left), cols(right), left.rows, right.rows)
...
def test_semantic_analyzer_warns_and_returns_empty_when_model_missing(caplog):
    ...
    with patch("spacy.load", side_effect=OSError("model not found")):
        with caplog.at_level(logging.WARNING, logger="tablemerge.analyzers"):
            result = analyzer.build_mapping(cols(left), cols(right), left.rows, right.rows)
    assert result == {}
