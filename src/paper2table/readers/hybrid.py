...
def build_instructions(schema):
    return (
        ...
        " If no title is present, generate a short, descriptive one.",
        " * first_page and last_page: annotate the consecutive range of pages"
        " (1-based) where it starts and where it ends.",
        ...
        "RESTRICTIONS",
        "============",
        " * Only consider data that appears in tabular form."
        " Ignore paragraphs or narrative text.",
        " * If a specific column from the COLUMN STRUCTURE is missing in the table,"
        " omit it from the output.",
        " * Focus only on tables relevant to the specified COLUMN STRUCTURE.",
        " * Ensure that extracted data and metadata are accurate and consistent"
